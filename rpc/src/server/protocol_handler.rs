...
pub async fn cycle(_: Request<Body>, params: Params, _: Query, env: RpcServiceEnvironment) -> ServiceResult {
    ...
    result_to_json_response(services::protocol::get_cycle_from_context_as_json(block_id, cycle_id, &env), env.log())
}
...
pub async fn get_contract_balance(_: Request<Body>, params: Params, _: Query, env: RpcServiceEnvironment) -> ServiceResult {
    ...
    result_to_json_response(
        services::protocol::proto_get_contract_balance(_chain_id, block_id, pkh, &env),
        env.log(),
    )
}
